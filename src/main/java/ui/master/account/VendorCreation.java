 ...
 */
public class VendorCreation extends javax.swing.JPanel {
     ...
     * @param conn
     */
    public VendorCreation(String companyCode, Connection conn) {
        initComponents();
    ...
    private void initComponents() {
        ...
        accountGroupCB.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "Purchase" }));
    ...
    private void saveButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_saveButtonActionPerformed
        if (validateUserInput()) {
            if (new AccountHandler().checkAccountNumberIsValidOrNot(companyCode, accountNumberTF.getText(), conn)) {
                if (new AccountHandler().saveVednorInformation(companyCode, this, conn)) {
    ...
    private void updateButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_updateButtonActionPerformed
        if (validateUserInput()) {
            if (accountNumber.equalsIgnoreCase(accountNumberTF.getText())) {
                if (new AccountHandler().updateVendorInformation(companyCode, this, accountNumber, conn)) {
                    this.enableAllField(false);
                    this.saveOrUpdateControlShow();
                }
            } else {
    ...
    private void editButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_editButtonActionPerformed
        if (new AccountHandler().fetchAccountInformation(companyCode, accountListTable, conn, "Vendor")) {
            this.add(list);
    ...
    private void accountListTableMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_accountListTableMouseClicked
        if (accountListTable.getRowCount() > 0) {
            if (evt.getClickCount() > 1 && accountListTable.getSelectedRow() > -1) {
                ...
                if (new AccountHandler().fetchDataOfVednor(companyCode, accountNumber, this, conn)) {
                    list.setVisible(false);
                    contentPanel.setVisible(true);
                    listControl.setVisible(false);
                    mainControl.setVisible(true);
