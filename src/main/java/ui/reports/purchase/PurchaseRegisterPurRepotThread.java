 ...
 * @author Vimal
 */
public class PurchaseRegisterPurRepotThread {
     ...
     * @param cal1
     */
    public PurchaseRegisterPurRepotThread(MainFrame mainFrame, JTable reportTable, ArrayList<String> selectedAccNo, String companyCode, Calendar cal, Calendar cal1) {
    ...
    private void doInBackgrounProcess() {
        ...
        String sql = "SELECT * from PURCHASE_REGISTER_DETAIL where companyCode = ? AND (dateForVoucher BETWEEN " + fromDate + " AND " + toDate + ") ORDER BY dateForVoucher desc";
        try {
            ...
            while (rs.next()) {
                ...
                if (new ReportUtils().checkDateIsValid(date, cal, cal1)) {
                    ...
                    if (selectedAccNo.contains(aN)) {
                        ...
                        reportTable.setValueAt(totA, rowCount, 5);
                        reportTable.setValueAt(totCr, rowCount, 6);
