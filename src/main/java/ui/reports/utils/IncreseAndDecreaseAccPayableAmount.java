 ...
 */
public class IncreseAndDecreaseAccPayableAmount {
    ...
    private double amountIncresePayable;
    private double amountDecreasePayable;

    public IncreseAndDecreaseAccPayableAmount(MainFrame mainFrame, String companyCode, Calendar cal, Calendar cal1) {
    ...
    private void doInBackground() {
        amountIncresePayable = 0.0;
        amountDecreasePayable = 0.0;
        ...
        //adding
        amountIncresePayable = amountIncresePayable + getFromSaleRegister();
        amountIncresePayable = amountIncresePayable + getFromPurchaseReturn();
        amountIncresePayable = amountIncresePayable + getFromSaleReturn();
        amountIncresePayable = amountIncresePayable + getFromPurchaseRegister();
        amountDecreasePayable = amountDecreasePayable + getFromPaymentVoucher();
        System.out.println("Account Increse Payable : " + amountIncresePayable);
        System.out.println("Account Decrease Payable : " + amountDecreasePayable);
        isFinished = true;
    }

    public double getAmountIncresePayable() {
        return amountIncresePayable;
    }

    public double getAmountDecreasePayable() {
        return amountDecreasePayable;
    }
    
    private double getFromSaleReturn() {
        String sqlR = "SELECT totalAmount,isBank,isCash,debitAmount FROM SALE_RETURN_DETAIL where companyCode = ? AND (dateForVoucher BETWEEN " + fromDate + " AND " + toDate + ")";
        String sqlB = "saleRet";
        return performOperationOnRegOrRet(sqlR, sqlB);
    }

    private double getFromSaleRegister() {
        String sqlR = "SELECT creditAmount FROM SALE_REGISTER_DETAIL where companyCode = ? AND creditAmount IS NOT NULL AND (dateForVoucher BETWEEN " + fromDate + " AND " + toDate + ")";
        String sqlB = "saleReg";
        return performOperationOnRegOrRet(sqlR, sqlB);
    }

    private double getFromPurchaseReturn() {
        String sqlR = "SELECT debitAmount FROM PURCHASE_RETURN_DETAIL where companyCode = ? AND debitAmount IS NOT NULL AND (dateForVoucher BETWEEN " + fromDate + " AND " + toDate + ")";
        String sqlB = "purRet";
        return performOperationOnRegOrRet(sqlR, sqlB);
    }

    private double getFromPurchaseRegister() {
        String sqlR = "SELECT totalAmount,isBank,isCash,creditAmount FROM PURCHASE_REGISTER_DETAIL where companyCode = ? AND (dateForVoucher BETWEEN " + fromDate + " AND " + toDate + ")";
        String sqlB = "purReg";
        return performOperationOnRegOrRet(sqlR, sqlB);
    }
    ...
    private double getFromPaymentVoucher() {
        ...
        String sqlS = "SELECT creditAmount FROM PAYMENT_VOUCHER_ACCOUNTS_DETAIL_FOR_CREDIT where voucherNumber = ?";
