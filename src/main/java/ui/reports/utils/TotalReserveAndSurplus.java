 ...
 */
public class TotalReserveAndSurplus {
     ...
     * @param cal1
     */
    public TotalReserveAndSurplus(MainFrame mainFrame, String companyCode, JTable firstReportTable, Calendar cal, Calendar cal1,int row,int col) {
    ...
    private void doInBackground() {
        double reserveAndSurplus = 0;
        ArrayList<String> forGroups = new ArrayList<>();
        forGroups.add("Reserves & Surplus");
        totJournalOnlyAmount = new JournalOnlyAmount(mainFrame, companyCode, cal, cal1, forGroups);
        reserveAndSurplus = reserveAndSurplus + totJournalOnlyAmount.run();
        if (totJournalOnlyAmount.isFinished()) {
            firstReportTable.setValueAt(reserveAndSurplus, row, col);
        }
    }
}
