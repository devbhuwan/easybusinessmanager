 ...
 */
public class SaleRegisterThread {
    ...
    public SaleRegisterThread(MainFrame mainFrame, JTable reportTable, ArrayList<String> selectedAccNo, String companyCode) {
    ...
    private void doInBackgroundProcess(Calendar cal, Calendar cal1) {
        ...
        String sql = "select * from SALE_REGISTER_DETAIL where companyCode = ? AND (dateForVoucher BETWEEN " + fromDate + " AND " + toDate + ") ORDER BY dateForVoucher desc";
        try {
            ...
            while (rs.next()) {
                ...
                if (new ReportUtils().checkDateIsValid(date, cal, cal1)) {
                    ...
                    if (selectedAccNo.contains(accNo)) {
                        ...
                        reportTable.setValueAt("Sale A/c", rowCount, 4);
                        reportTable.setValueAt(null, rowCount, 5);
                        reportTable.setValueAt(totAmt, rowCount, 6);
                        reportTable.setValueAt("SALE REGISTER", rowCount, 7);
                        rowCount++;
                    }
                    ...
                    if (selectedAccNo.contains(accNo)) {
                        if (!b1 && !b2) {
                            ...
                            try {
                                ...
                                reportTable.setValueAt(accName, rowCount, 4);
                                reportTable.setValueAt(null, rowCount, 5);
                                reportTable.setValueAt(creditAmt, rowCount, 6);
                                reportTable.setValueAt("SALE REGISTER", rowCount, 7);
                                rowCount++;
                            ...
                            try {
                                ...
                                reportTable.setValueAt(accNo, rowCount, 3);
                                reportTable.setValueAt(accName, rowCount, 4);
                                reportTable.setValueAt(debitAmt, rowCount, 5);
                                reportTable.setValueAt(null, rowCount, 6);
                                reportTable.setValueAt("SALE REGISTER", rowCount, 7);
                                rowCount++;
                            } catch (NullPointerException | NumberFormatException ex) {
    ...
    private boolean checkCashPay(String vN, int row, Date date) {
        ...
        String sql = "SELECT * FROM SALE_REGISTER_CASH_DETAIL where companyCode = ? AND voucherNumber = ?";
        try {
            ...
            if (r.next()) {
                ...
                if (selectedAccNo.contains(accNo)) {
                    ...
                    reportTable.setValueAt(amt, rowCount, 5);
                    reportTable.setValueAt(null, rowCount, 6);
                    reportTable.setValueAt("SALE REGISTER", rowCount, 7);
                    rowCount++;
                }
    ...
    private boolean checkBankPay(String vN, int row, Date date) {
        boolean status = false;
        String sql = "SELECT * FROM SALE_REGISTER_BANK_DETAIL where companyCode = ? AND voucherNumber = ?";
        try {
            con = mainFrame.getConnection2();
            con.setAutoCommit(false);
            p = con.prepareStatement(sql);
