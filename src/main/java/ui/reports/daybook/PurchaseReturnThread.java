 ...
 */
public class PurchaseReturnThread {
    ...
    public PurchaseReturnThread(MainFrame mainFrame, JTable reportTable, String companyCode) {
    ...
    private void doInBackgroundProcess(Calendar cal) {
        ...
        String sql = "select * from PURCHASE_RETURN_DETAIL where companyCode = ? AND (dateForVoucher BETWEEN " + fromDate + " AND " + toDate + ")";
        try {
            ...
            while (rs.next()) {
                ...
                if (new ReportUtils().checkDateIsValid(date, cal, cal)) {
                    ...
                    reportTable.setValueAt("Purchase Return A/c", rowCount, 4);
    ...
    private boolean checkCashPay(String vN, int row, Date date) {
        ...
        String sql = "SELECT * FROM PURCHASE_RETURN_CASH_DETAIL where companyCode = ? AND voucherNumber = ?";
        try {
    ...
    private boolean checkBankPay(String vN, int row, Date date) {
        boolean status = false;
        String sql = "SELECT * FROM PURCHASE_RETURN_BANK_DETAIL where companyCode = ? AND voucherNumber = ?";
        try {
            con = mainFrame.getConnection2();
            con.setAutoCommit(false);
            p = con.prepareStatement(sql);
