 ...
 */
public class SaleRegisterCashThread {
    ...
    public SaleRegisterCashThread(MainFrame mainFrame, JTable reportTable, ArrayList<String> selectedAccNo, String companyCode) {
    ...
    private void doInBackgroundProcess(Calendar cal, Calendar cal1) {
        ...
        String sql = "select voucherNumber,accountNumber,dateForVoucher from SALE_REGISTER_DETAIL where companyCode = ? AND isCash = 'Y' AND (dateForVoucher BETWEEN " + fromDate + " AND " + toDate + ") ORDER BY dateForVoucher desc";
        try {
            ...
            while (rs.next()) {
                ...
                if(new ReportUtils().checkDateIsValid(date, cal, cal1)) {
                    String sqlQ = "SELECT amount,accountNumber FROM SALE_REGISTER_CASH_DETAIL where companyCode = ? AND voucherNumber = ?";
                try {
