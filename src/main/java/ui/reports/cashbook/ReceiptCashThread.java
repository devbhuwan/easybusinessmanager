 ...
 */
public class ReceiptCashThread {
    ...
    public ReceiptCashThread(MainFrame mainFrame, JTable reportTable, ArrayList<String> selectedAccNo, String companyCode) {
    ...
    private void doInBackgroundProcess(Calendar cal, Calendar cal1) {
        ...
        String sql = "select voucherNumber,dateForVoucher from RECEIPT_VOUCHER_DETAIL where companyCode = ? AND (dateForVoucher BETWEEN " + fromDate + " AND " + toDate + ") ORDER by dateForVoucher desc";
        try {
    ...
    private void checkAccounts(String vN, String date) {
        String sql = "select * from RECEIPT_VOUCHER_ACCOUNTS_DETAIL_FOR_DEBIT,RECEIPT_VOUCHER_ACCOUNTS_DETAIL_FOR_CREDIT where (RECEIPT_VOUCHER_ACCOUNTS_DETAIL_FOR_CREDIT.voucherNumber = ? ) AND (RECEIPT_VOUCHER_ACCOUNTS_DETAIL_FOR_DEBIT.voucherNumber = ?) AND (RECEIPT_VOUCHER_ACCOUNTS_DETAIL_FOR_CREDIT.ID = RECEIPT_VOUCHER_ACCOUNTS_DETAIL_FOR_DEBIT.ID)";
        try {
            ...
            r = p.executeQuery();
            while (r.next()) {
                String aNC = r.getString("debitAccountNumber");
                if (selectedAccNo.contains(aNC)) {
                    String amountC = r.getString("debitAmount");
                    String sNo = r.getString("creditAccountNumber");
                    ...
                    reportTable.setValueAt("RECEIPT VOUCHER", rowCount, 7);
                    rowCount++;
                }
            }
            con.commit();
