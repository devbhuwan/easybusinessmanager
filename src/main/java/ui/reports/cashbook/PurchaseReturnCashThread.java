 ...
 */
public class PurchaseReturnCashThread {
    ...
    public PurchaseReturnCashThread(MainFrame mainFrame, JTable reportTable, ArrayList<String> selectedAccNo, String companyCode) {
    ...
    private void doInBackgroundProcess(Calendar cal, Calendar cal1) {
        ...
        String sql = "select voucherNumber,accountNumber,dateForVoucher from PURCHASE_RETURN_DETAIL where companyCode = ? AND isCash = 'Y' AND (dateForVoucher BETWEEN " + fromDate + " AND " + toDate + ") ORDER BY dateForVoucher desc";
        try {
            ...
            while (rs.next()) {
                ...
                String sNo = rs.getString("accountNumber");
                Date date = rs.getDate("dateForVoucher");
                if (new ReportUtils().checkDateIsValid(date, cal, cal1)) {
                    String sqlQ = "SELECT amount,accountNumber FROM PURCHASE_RETURN_CASH_DETAIL where companyCode = ? AND voucherNumber = ?";
                    try {
                        con = mainFrame.getConnection2();
                        con.setAutoCommit(false);
                        p = con.prepareStatement(sqlQ);
                        ...
                        if (r.next()) {
                            ...
                            if (selectedAccNo.contains(accNo)) {
                                addRow();
                                String accName = new ReportUtils().getAccountName(sNo, companyCode, mainFrame);
                                ...
                                reportTable.setValueAt("PURCHASE RETURN", rowCount, 7);
                                rowCount++;
                            }
                        }
                        con.commit();
