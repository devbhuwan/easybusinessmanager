 ...
 */
public class SaleRegisterBankThread {
    ...
    public SaleRegisterBankThread(MainFrame mainFrame, JTable reportTable, ArrayList<String> selectedAccNo, String companyCode) {
    ...
    private void doInBackgroundProcess(Calendar cal, Calendar cal1) {
        ...
        String sql = "select voucherNumber,accountNumber,dateForVoucher from SALE_REGISTER_DETAIL where companyCode = ? AND isBank = 'Y' AND (dateForVoucher BETWEEN " + fromDate + " AND " + toDate + ") ORDER by dateForVoucher desc";
        try {
            ...
            while (rs.next()) {
                ...
                if(new ReportUtils().checkDateIsValid(date, cal, cal1)) {
                    String sqlQ = "SELECT amount,accountNumber FROM SALE_REGISTER_BANK_DETAIL where companyCode = ? AND voucherNumber = ?";
                try {
