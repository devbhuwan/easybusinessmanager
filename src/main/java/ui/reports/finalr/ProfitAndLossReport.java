...
import ui.reports.print.ProfitAndLossNotesTaker;
import ui.reports.print.action.FinalReportPrint;
import ui.reports.utils.TotalGrossLossOrProfit;
 ...
 * @author Vimal
 */
public class ProfitAndLossReport extends javax.swing.JFrame implements TableModelListener {
    ...
    private ProfitAndLossNotesTaker notesTaker;
     ...
     * @param dialog
     */
    public ProfitAndLossReport(MainFrame mainFrame, String companyCode, DateWiseReportDialog dialog) {
        initComponents();
    ...
    private void initComponents() {
        ...
        setTitle("Profit And Loss Report");
        ...
        jLabel3.setText("Profit And Loss Report");
        ...
        firstReportTable.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
                {null, "", null, null, "", null},
                { new Integer(1), "Indirect Expenses", null,  new Integer(1), "Incomes", null},
    ...
    private void printButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_printButtonActionPerformed
         if (firstReportTable.getRowCount() > 2) {
            notesTaker = new ProfitAndLossNotesTaker(this, true);
    ...
    public void fetchInfo() {
        ...
        new Thread(new Runnable() {
            private TotalGrossLossOrProfit totGrossLOrP;
            private TotalIncomes totIncomes;
            private TotalIndirectExpenses totInDirectExpenses;

            @Override
            public void run() {
                totGrossLOrP = new TotalGrossLossOrProfit(mainFrame, companyCode, firstReportTable, cal, cal1);
                totInDirectExpenses = new TotalIndirectExpenses(mainFrame, companyCode, firstReportTable, cal, cal1, 1, 2);
                totIncomes = new TotalIncomes(mainFrame, companyCode, firstReportTable, cal, cal1, 1, 5);
                totGrossLOrP.runIn();
                totInDirectExpenses.runIn();
                totIncomes.runIn();
    ...
    public void tableChanged(TableModelEvent e) {
        int leftCounter = 0;
        int rightCounter = 0;
        if (firstReportTable.getValueAt(0, 1) != null) {
            if (firstReportTable.getValueAt(1, 2) != null && firstReportTable.getValueAt(1, 5) != null) {
                loadingLabel.setVisible(false);
                firstPanelScrollPane.setVisible(true);
                reloadButton.setVisible(true);
            }
        } else if (firstReportTable.getValueAt(0, 4) != null) {
            if (firstReportTable.getValueAt(1, 2) != null && firstReportTable.getValueAt(1, 5) != null) {
                loadingLabel.setVisible(false);
                firstPanelScrollPane.setVisible(true);
                reloadButton.setVisible(true);
    ...
    private void setFirstTableTotalValues() {
        ...
        if (debit > credit) {
            firstReportTable.setValueAt("NET LOSS (c/d)", 2, 4);
            firstReportTable.setValueAt(sepAmount, 2, 5);
            firstReportTable.setValueAt(debit, 3, 2);
            firstReportTable.setValueAt(credit + sepAmount, 3, 5);
        } else if (credit > debit) {
            firstReportTable.setValueAt("NET PROFIT (c/d)", 2, 1);
            firstReportTable.setValueAt(sepAmount, 2, 2);
            firstReportTable.setValueAt(debit + sepAmount, 3, 2);
            firstReportTable.setValueAt(credit, 3, 5);
        } else {
            firstReportTable.setValueAt(null, 2, 0);
            firstReportTable.setValueAt(null, 2, 1);
            firstReportTable.setValueAt(null, 2, 2);
            firstReportTable.setValueAt(null, 2, 3);
            firstReportTable.setValueAt(null, 2, 4);
            firstReportTable.setValueAt(null, 2, 5);
            firstReportTable.setValueAt(debit, 3, 2);
            firstReportTable.setValueAt(credit, 3, 5);
        }
    }
     ...
     public void printTable(String notes, String reportName) {
        ...
        if (Files.exists(path)) {
            if (Files.exists(pathForReport)) {
                ...
                if (optionPane == 0) {
                    this.notes = notes;
                    print = new FinalReportPrint(mainFrame, companyCode, firstReportTable, notes, cal, cal1, reportName, "PROFIT AND LOSS");
                    print.prepare();
                    print.print();
                    notesTaker.dispose();
                } else {
