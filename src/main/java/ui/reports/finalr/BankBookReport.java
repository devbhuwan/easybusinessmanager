...
import ui.reports.bankbook.BankBookUtils;
import ui.reports.print.BankNotesTaker;
 ...
 * @author Vimal
 */
public class BankBookReport extends javax.swing.JFrame implements TableModelListener {

    private final MainFrame mainFrame;
    private final String companyCode;
    private Calendar cal;
    private Calendar cal1;
    private final AccountWiseReportDialog dialog;
    private ArrayList<String> accNo;
    private BankBookUtils jru;
    private BankNotesTaker notesTaker;
     ...
     * @param dialog
     */
    public BankBookReport(MainFrame mainFrame, String companyCode, AccountWiseReportDialog dialog) {
        initComponents();
    ...
    private void printButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_printButtonActionPerformed
        if (firstReportTable.getRowCount() > 2) {
            notesTaker = new BankNotesTaker(this, true);
    ...
    public void fetchInfo(ArrayList<String> selectedAccNo) {
        accNo = selectedAccNo;
        ...
        new Thread(new Runnable() {
            ...
            @Override
            public void run() {
                jru = new BankBookUtils(mainFrame, cal, cal1, firstReportTable, accNo, companyCode);
                jru.run();
    ...
    private void setFirstTableTotalValues() {
        ...
        firstReportTable.setValueAt("GRAND TOTAL", firstReportTable.getRowCount() - 2, 4);
        firstReportTable.setValueAt(debit, firstReportTable.getRowCount() - 2, 5);
        firstReportTable.setValueAt(credit, firstReportTable.getRowCount() - 2, 6);
        firstReportTable.setValueAt("BANK BALANCE", firstReportTable.getRowCount() - 1, 5);
        firstReportTable.setValueAt(credit > debit ? (credit - debit) : null, firstReportTable.getRowCount() - 1, 6);
        int count = 1;
        for (int i = 0; i < firstReportTable.getRowCount() - 2; i++) {
    ...
    public void printTable(String notes, String reportName) {
        ...
        if (Files.exists(path)) {
            if (Files.exists(pathForReport)) {
                ...
                if (optionPane == 0) {
                    this.notes = notes;
                    print = new AccountWiseReport2Print(mainFrame, companyCode, firstReportTable, notes, cal, cal1, reportName, "BANK BOOK");
                    print.prepare();
                    print.print();
                    notesTaker.dispose();
                } else {
