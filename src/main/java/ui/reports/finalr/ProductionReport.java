...
import ui.reports.print.ProductionReportNotesTaker;
import ui.reports.print.action.ProductionReportPrint;
import ui.reports.production.ProductionReportUtils;
 ...
 * @author Vimal
 */
public class ProductionReport extends javax.swing.JFrame implements TableModelListener {

    private final MainFrame mainFrame;
    private final String companyCode;
    private Calendar cal;
    private Calendar cal1;
    private final ItemWiseReportDialog dialog;
    private ArrayList<String> itemCode;
    private ProductionReportUtils jru;
    private ProductionReportNotesTaker notesTaker;
    private String notes;
    private ProductionReportPrint print;
     ...
     * @param dialog
     */
    public ProductionReport(MainFrame mainFrame, String companyCode, ItemWiseReportDialog dialog) {
        initComponents();
    ...
    private void initComponents() {
        ...
        jLabel3.setText("Production Report");
        ...
        firstReportTable.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "S. N.", "Date", "Voucher Number ", "Item Code", "Item Name", "Unit", "Quantity", "Rate", "Amount", "Expiry Date"
            }
        ) {
        ...
        if (firstReportTable.getColumnModel().getColumnCount() > 0) {
            ...
            firstReportTable.getColumnModel().getColumn(7).setPreferredWidth(400);
            firstReportTable.getColumnModel().getColumn(8).setPreferredWidth(500);
            firstReportTable.getColumnModel().getColumn(9).setPreferredWidth(500);
        }
    ...
    private void printButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_printButtonActionPerformed
        if (firstReportTable.getRowCount() > 2) {
            notesTaker = new ProductionReportNotesTaker(this, true);
    ...
    public void fetchInfo(ArrayList<String> iC) {
        itemCode = iC;
        ...
        new Thread(new Runnable() {
            ...
            @Override
            public void run() {
                jru = new ProductionReportUtils(mainFrame, cal, cal1, firstReportTable,itemCode, companyCode);
                jru.run();
    ...
    private void setFirstTableTotalValues() {
        ...
        for (int i = 0; i < firstReportTable.getRowCount() - 2; i++) {
            try {
                amount = amount + Double.valueOf(firstReportTable.getValueAt(i, 8).toString());
            } catch (NumberFormatException | NullPointerException ex) {
            }
        }
        firstReportTable.setValueAt("GRAND TOTAL", firstReportTable.getRowCount() - 1, 5);
        firstReportTable.setValueAt(amount, firstReportTable.getRowCount() - 1, 8);
        int count = 1;
        for (int i = 0; i < firstReportTable.getRowCount() - 2; i++) {
    ...
    public void printTable(String notes, String reportName) {
        ...
        if (Files.exists(path)) {
            if (Files.exists(pathForReport)) {
                ...
                if (optionPane == 0) {
                    this.notes = notes;
                    print = new ProductionReportPrint(mainFrame, companyCode, firstReportTable, notes, cal, cal1, reportName, "PRODUCTION REPORT");
                    print.prepare();
                    print.print();
                    notesTaker.dispose();
                } else {
