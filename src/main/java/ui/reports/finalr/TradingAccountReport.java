...
import ui.reports.print.TradingNotesTaker;
 ...
 * @author Vimal
 */
public class TradingAccountReport extends javax.swing.JFrame implements TableModelListener {
    ...
    private TradingNotesTaker notesTaker;
     ...
     * @param dialog
     */
    public TradingAccountReport(MainFrame mainFrame, String companyCode, DateWiseReportDialog dialog) {
        initComponents();
    ...
    private void initComponents() {
        ...
        setTitle("Trading Account Report");
        ...
        jLabel3.setText("Trading Account Report");
        ...
        firstReportTable.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
                { new Integer(1), "Opening Stock", null,  new Integer(1), "Sales", null},
                { new Integer(2), "Purchases", null,  new Integer(2), "Closing Stock", null},
                { new Integer(3), "Direct Expenses", null, null, "", null},
    ...
    private void printButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_printButtonActionPerformed
         if (firstReportTable.getRowCount() > 2) {
            notesTaker = new TradingNotesTaker(this, true);
    ...
    public void fetchInfo() {
        ...
        new Thread(new Runnable() {
            ...
            private TotalClosingStock totClosingStock;

            @Override
            public void run() {
                totOpeningStock = new TotalOpeningStock(mainFrame, companyCode, firstReportTable);
                totPurchase = new TotalPurchase(mainFrame, companyCode, firstReportTable, cal, cal1, 1, 2);
                totDirectExpenses = new TotalDirectExpenses(mainFrame, companyCode, firstReportTable, cal, cal1,2,2);
                totSale = new TotalSale(mainFrame, companyCode, firstReportTable, cal, cal1, 0, 5);
                totClosingStock = new TotalClosingStock(mainFrame, companyCode, firstReportTable, cal, cal1, 1, 5);
                totOpeningStock.runIn();
                totPurchase.runIn();
                totDirectExpenses.runIn();
                totSale.runIn();
                totClosingStock.runIn();
    ...
    private void setFirstTableTotalValues() {
        ...
        if (debit > credit) {
            firstReportTable.setValueAt("GROSS LOSS (c/d)", 3, 4);
            firstReportTable.setValueAt(sepAmount, 3, 5);
            firstReportTable.setValueAt(debit, 4, 2);
            firstReportTable.setValueAt(credit + sepAmount, 4, 5);
        } else if (credit > debit) {
            firstReportTable.setValueAt("GROSS PROFIT (c/d)", 3, 1);
            firstReportTable.setValueAt(Math.abs(credit - debit), 3, 2);
            firstReportTable.setValueAt(debit + sepAmount, 4, 2);
            firstReportTable.setValueAt(credit, 4, 5);
        } else {
            firstReportTable.setValueAt(null, 3, 0);
            firstReportTable.setValueAt(null, 3, 1);
            firstReportTable.setValueAt(null, 3, 2);
            firstReportTable.setValueAt(null, 3, 3);
            firstReportTable.setValueAt(null, 3, 4);
            firstReportTable.setValueAt(null, 3, 5);
            firstReportTable.setValueAt(debit, 4, 2);
            firstReportTable.setValueAt(credit, 4, 5);
        }
    }
     ...
     public void printTable(String notes, String reportName) {
        ...
        if (Files.exists(path)) {
            if (Files.exists(pathForReport)) {
                ...
                if (optionPane == 0) {
                    this.notes = notes;
                    print = new FinalReportPrint(mainFrame, companyCode, firstReportTable, notes, cal, cal1, reportName, "TRADING ACCOUNT");
                    print.prepare();
                    print.print();
                    notesTaker.dispose();
                } else {
