...
import ui.reports.print.StocksReportNotesTaker;
import ui.reports.print.action.StockReportPrint;
import ui.reports.selector.ItemWiseReportDialog;
import ui.reports.stock.StocksReportUtils;

/**
 *
 * @author Vimal
 */
public class StocksReport extends javax.swing.JFrame implements TableModelListener {
    ...
    private StocksReportUtils jru;
    private StocksReportNotesTaker notesTaker;
    private String notes;
    private StockReportPrint print;
     ...
     * @param dialog
     */
    public StocksReport(MainFrame mainFrame, String companyCode, ItemWiseReportDialog dialog) {
        initComponents();
    ...
    private void initComponents() {
        ...
        firstReportTable.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "S. N.", "Item Code", "Item Name", "Unit", "Quantity", "Extra Quantity", "Rate", "Amount"
            }
        ) {
        ...
        if (firstReportTable.getColumnModel().getColumnCount() > 0) {
            ...
            firstReportTable.getColumnModel().getColumn(1).setPreferredWidth(400);
            firstReportTable.getColumnModel().getColumn(2).setPreferredWidth(500);
            firstReportTable.getColumnModel().getColumn(3).setPreferredWidth(400);
            firstReportTable.getColumnModel().getColumn(4).setPreferredWidth(400);
    ...
    private void printButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_printButtonActionPerformed
        if (firstReportTable.getRowCount() > 2) {
            notesTaker = new StocksReportNotesTaker(this, true);
    ...
    public void fetchInfo(ArrayList<String> iC) {
        ...
        new Thread(new Runnable() {
            ...
            @Override
            public void run() {
                jru = new StocksReportUtils(mainFrame, cal, cal1, firstReportTable,itemCode, companyCode);
                jru.run();
    ...
    private void setFirstTableTotalValues() {
        ...
        for (int i = 0; i < firstReportTable.getRowCount() - 2; i++) {
            try {
                amount = amount + Double.valueOf(firstReportTable.getValueAt(i, 7).toString());
            } catch (NumberFormatException | NullPointerException ex) {
            }
        }
        firstReportTable.setValueAt("GRAND TOTAL", firstReportTable.getRowCount() - 1, 6);
        firstReportTable.setValueAt(amount, firstReportTable.getRowCount() - 1, 7);
        int count = 1;
        for (int i = 0; i < firstReportTable.getRowCount() - 2; i++) {
    ...
    public void printTable(String notes, String reportName) {
        ...
        if (Files.exists(path)) {
            if (Files.exists(pathForReport)) {
                ...
                if (optionPane == 0) {
                    this.notes = notes;
                    print = new StockReportPrint(mainFrame, companyCode, firstReportTable, notes, cal, cal1, reportName, "STOCK REPORT");
                    print.prepare();
                    print.print();
                    notesTaker.dispose();
                } else {
