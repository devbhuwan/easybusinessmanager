...
import ui.reports.print.SaleRegisterNotesTaker;
...
import ui.voucher.saleandpurchase.utils.SaleRegisterHandler;

/**
 *
 * @author Vimal
 */
public class SaleRegister extends javax.swing.JPanel {
    ...
    private SaleRegisterNotesTaker notesTaker;
     ...
     * @param conn
     */
    public SaleRegister(MainFrame mainFrame, String companyCode, Connection conn) {
        initComponents();
    ...
    private void saveButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_saveButtonActionPerformed
        if (validateUserInput()) {
            if (new SaleRegisterHandler(mainFrame, this).checkVoucherNumberIsValidOrNot(companyCode, voucherNumberTF.getText(), conn)) {
                if (new SaleRegisterHandler(mainFrame, this).saveSaleRegisterInfo(this, conn, companyCode)) {
                    if (new SaleRegisterHandler(mainFrame, this).saveItemOrderInfo(this, conn, companyCode, itemTable)) {
                        if (billSundryTable.getRowCount() > 1) {
                            if (new SaleRegisterHandler(mainFrame, this).saveBillSundryInfo(this, conn, companyCode, billSundryTable)) {
    ...
    private void updateButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_updateButtonActionPerformed
        if (validateUserInput()) {
            if (voucherNumberTF.getText().equals(String.valueOf(voucherNumber))) {
                if (new SaleRegisterHandler(mainFrame, this).deleteVoucherNumber(companyCode, voucherNumber, conn)) {
                    if (new SaleRegisterHandler(mainFrame, this).saveSaleRegisterInfo(this, conn, companyCode)) {
                        if (new SaleRegisterHandler(mainFrame, this).saveItemOrderInfo(this, conn, companyCode, itemTable)) {
                            if (billSundryTable.getRowCount() > 1) {
                                if (new SaleRegisterHandler(mainFrame, this).saveBillSundryInfo(this, conn, companyCode, billSundryTable)) {
    ...
    private void editButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_editButtonActionPerformed
        if (new SaleRegisterHandler(mainFrame, this).fetchSaleRegisterInformation(companyCode, saleOrderListTable, conn)) {
            this.add(list);
    ...
    private void deleteButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_deleteButtonActionPerformed
        if (new SaleRegisterHandler(mainFrame, this).deleteVoucherNumber(companyCode, voucherNumberTF.getText(), conn)) {
            this.clearAllField();
            this.enableAllField(false);
    ...
    private void printButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_printButtonActionPerformed
        notesTaker = new SaleRegisterNotesTaker(mainFrame, this, true);
    ...
    private void saleOrderListTableMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_saleOrderListTableMouseClicked
        if (saleOrderListTable.getRowCount() > 0) {
            if (evt.getClickCount() > 1 && saleOrderListTable.getSelectedRow() > -1) {
                ...
                if (new SaleRegisterHandler(mainFrame, this).fetchDataOfSaleRegister(companyCode, voucherNumber, this, conn)) {
                    if (new SaleRegisterHandler(mainFrame, this).fetchItemDetail(companyCode, voucherNumber, itemTable, this, conn)) {
                        if (new SaleRegisterHandler(mainFrame, this).fetchBillSundryDetail(companyCode, voucherNumber, billSundryTable, this, conn)) {
                            list.setVisible(false);
                            container.setVisible(true);
                            contolLP.setVisible(true);
                            this.remove(list);
    ...
    public void setVoucherNumber(String companyCode, Connection conn) {
        voucherNumberTF.setText(new FetchVoucherData().getNextSaleRegisterVoucherNo(companyCode, conn));
    }
    ...
    private void itemTableControl() {
        ...
        if (selectedRow > -1) {
            if (itemTable.getSelectedRow() == itemTable.getRowCount() - 1) {
                ...
                calculateAmount();
                if (itemTable.getModel().getValueAt(itemTable.getSelectedRow(), 1) != null) {
                    primaryUnit = new SaleRegisterHandler(mainFrame, this).getUnitName(itemTable.getModel().getValueAt(itemTable.getSelectedRow(), 1).toString(), companyCode, conn);
