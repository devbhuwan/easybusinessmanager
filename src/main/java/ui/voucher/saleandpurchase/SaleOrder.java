...
import ui.voucher.saleandpurchase.utils.SaleOrderHandler;

/**
 *
 * @author Vimal
 */
public class SaleOrder extends javax.swing.JPanel {
    ...
    /**
     * Creates new form SaleOrder
     *
     * @param mainFrame
     * @param companyCode
     * @param conn
     */
    public SaleOrder(MainFrame mainFrame, String companyCode, Connection conn) {
        initComponents();
    ...
    private void saveButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_saveButtonActionPerformed
        if (validateUserInput()) {
            if (new SaleOrderHandler(mainFrame, this).checkVoucherNumberIsValidOrNot(companyCode, voucherNumberTF.getText(), conn)) {
                if (new SaleOrderHandler(mainFrame, this).saveSaleOrderInfo(this, conn, companyCode)) {
                    if (new SaleOrderHandler(mainFrame, this).saveItemOrderInfo(this, conn, companyCode, itemTable)) {
                        if (billSundryTable.getRowCount() > 1) {
                            if (new SaleOrderHandler(mainFrame, this).saveBillSundryInfo(this, conn, companyCode, billSundryTable)) {
    ...
    private void updateButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_updateButtonActionPerformed
        if (validateUserInput()) {
            if (voucherNumberTF.getText().equals(String.valueOf(voucherNumber))) {
                if (new SaleOrderHandler(mainFrame, this).deleteVoucherNumber(companyCode, voucherNumber, conn)) {
                    if (new SaleOrderHandler(mainFrame, this).saveSaleOrderInfo(this, conn, companyCode)) {
                        if (new SaleOrderHandler(mainFrame, this).saveItemOrderInfo(this, conn, companyCode, itemTable)) {
                            if (billSundryTable.getRowCount() > 1) {
                                if (new SaleOrderHandler(mainFrame, this).saveBillSundryInfo(this, conn, companyCode, billSundryTable)) {
    ...
    private void editButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_editButtonActionPerformed
        if (new SaleOrderHandler(mainFrame, this).fetchSaleOrderInformation(companyCode, saleOrderListTable, conn)) {
            this.add(list);
    ...
    private void deleteButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_deleteButtonActionPerformed
        if (new SaleOrderHandler(mainFrame, this).deleteVoucherNumber(companyCode, voucherNumberTF.getText(), conn)) {
            this.clearAllField();
            this.enableAllField(false);
    ...
    private void saleOrderListTableMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_saleOrderListTableMouseClicked
        if (saleOrderListTable.getRowCount() > 0) {
            if (evt.getClickCount() > 1 && saleOrderListTable.getSelectedRow() > -1) {
                ...
                if (new SaleOrderHandler(mainFrame, this).fetchDataOfSaleOrder(companyCode, voucherNumber, this, conn)) {
                    if (new SaleOrderHandler(mainFrame, this).fetchItemDetail(companyCode, voucherNumber, itemTable, this, conn)) {
                        if (new SaleOrderHandler(mainFrame, this).fetchBillSundryDetail(companyCode, voucherNumber, billSundryTable, this, conn)) {
                            list.setVisible(false);
                            container.setVisible(true);
                            contolLP.setVisible(true);
                            this.remove(list);
    ...
    public void setVoucherNumber(String companyCode, Connection conn) {
        voucherNumberTF.setText(new FetchVoucherData().getNextSaleOrderVoucherNo(companyCode, conn));
    }
        ...
        if (selectedRow > -1) {
            if (itemTable.getSelectedRow() == itemTable.getRowCount() - 1) {
                ...
                if (itemTable.getModel().getValueAt(itemTable.getSelectedRow(), 1) != null) {
                    primaryUnit = new SaleOrderHandler(mainFrame, this).getUnitName(itemTable.getModel().getValueAt(itemTable.getSelectedRow(), 1).toString(), companyCode, conn);
