...
public class PurchaseOrderHandler {
    ...
    private final PurchaseOrder saleOrder;
     ...
     * @param saleOrder
     */
    public PurchaseOrderHandler(MainFrame mainFrame, PurchaseOrder saleOrder) {
    ...
    public boolean fetchPurchaseOrderInformation(String companyCode, JTable saleOrderListTable, Connection conn) {
        boolean status = false;
        removeAllRows(saleOrderListTable);
        String sql = "select * from PURCHASE_ORDER_DETAIL where companyCode = ?";
    ...
    public boolean checkVoucherNumberIsValidOrNot(String companyCode, String voucherNumber, Connection conn) {
        boolean status = true;
        String sql = "select voucherNumber from PURCHASE_ORDER_DETAIL where voucherNumber = ? AND companyCode = ?";
        try {
            conn.setAutoCommit(false);
            ps = conn.prepareStatement(sql);
    ...
    public boolean savePurchaseOrderInfo(PurchaseOrder aThis, Connection conn, String companyCode) {
        boolean status = false;
        String sql1 = "insert into PURCHASE_ORDER_DETAIL values (?,?,?,?,?,?,?,?)";
        try {
            conn.setAutoCommit(false);
    ...
    public boolean saveItemOrderInfo(PurchaseOrder aThis, Connection conn, String companyCode, JTable itemTable) {
        boolean status = false;
        String sql2 = "insert into PURCHASE_ORDER_ITEM_LIST (itemCode,unitName,quantity,rate,itemAmount,billSundry,taxClass,atTheRate,nature,miscAmount,totalRowAmount,voucherNumber,companyCode) values (?,?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    public boolean saveBillSundryInfo(PurchaseOrder aThis, Connection conn, String companyCode, JTable billSundryTable) {
        boolean status = false;
        String sql = "insert into PURCHASE_ORDER_BILL_SUNDRY_LIST (billSundry,atTheRate,taxClass,nature,amount,voucherNumber,companyCode) values (?,?,?,?,?,?,?)";
    ...
    public boolean deleteVoucherNumber(String companyCode, String voucherNumber, Connection conn) {
        boolean status = false;
        String sql = "delete from PURCHASE_ORDER_DETAIL where voucherNumber = ? AND companyCode = ?";
        try {
            conn.setAutoCommit(false);
            ps = conn.prepareStatement(sql);
            ps.setString(1, voucherNumber);
    ...
    public boolean fetchDataOfPurchaseOrder(String companyCode, String voucherNumber, PurchaseOrder aThis, Connection conn) {
        boolean status = false;
        String sql = "select * from PURCHASE_ORDER_DETAIL where voucherNumber = ? AND companyCode = ?";
        try {
            conn.setAutoCommit(false);
            ps = conn.prepareStatement(sql);
            ps.setString(1, voucherNumber);
    ...
    public boolean fetchItemDetail(String companyCode, String voucherNumber, JTable itemTable, PurchaseOrder aThis, Connection conn) {
        boolean status = false;
        String sql = "select * from PURCHASE_ORDER_ITEM_LIST where voucherNumber = ? AND companyCode = ?";
        try {
            conn.setAutoCommit(false);
            ps = conn.prepareStatement(sql);
            ps.setString(1, voucherNumber);
    ...
    public boolean fetchBillSundryDetail(String companyCode, String voucherNumber, JTable billSundryTable, PurchaseOrder aThis, Connection conn) {
        boolean status = false;
        String sql = "select * from PURCHASE_ORDER_BILL_SUNDRY_LIST where voucherNumber = ? AND companyCode = ?";
        try {
            conn.setAutoCommit(false);
            ps = conn.prepareStatement(sql);
            ps.setString(1, voucherNumber);
