 ...
 */
public class PurchaseRegisterHandler {

    private PreparedStatement ps;
    private ResultSet rs;
    private final MainFrame mainFrame;
    private final PurchaseRegister saleOrder;
     ...
     * @param saleOrder
     */
    public PurchaseRegisterHandler(MainFrame mainFrame, PurchaseRegister saleOrder) {
    ...
    public boolean fetchPurchaseRegisterInformation(String companyCode, JTable saleOrderListTable, Connection conn) {
        boolean status = false;
        removeAllRows(saleOrderListTable);
        String sql = "select * from PURCHASE_REGISTER_DETAIL where companyCode = ?";
        ...
        try {
            ...
            Logger.getLogger(PurchaseRegisterHandler.class.getName()).log(Level.SEVERE, null, ex);
        }
        return status;
    }
    ...
    public boolean checkVoucherNumberIsValidOrNot(String companyCode, String voucherNumber, Connection conn) {
        boolean status = true;
        String sql = "select voucherNumber from PURCHASE_REGISTER_DETAIL where voucherNumber = ? AND companyCode = ?";
        try {
            conn.setAutoCommit(false);
            ps = conn.prepareStatement(sql);
    ...
    public boolean savePurchaseRegisterInfo(PurchaseRegister aThis, Connection conn, String companyCode) {
        boolean status = false;
        String sql1 = "insert into PURCHASE_REGISTER_DETAIL values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        try {
            conn.setAutoCommit(false);
        ...
        if (aThis.getIsCash().equalsIgnoreCase("Y")) {
            status = false;
            String sql = "insert into PURCHASE_REGISTER_CASH_DETAIL (amount,accountNumber,voucherNumber,companyCode) values (?,?,?,?)";
            try {
                conn.setAutoCommit(false);
                ps = conn.prepareStatement(sql);
        ...
        if (aThis.getIsBank().equalsIgnoreCase("Y")) {
            status = false;
            String sql = "insert into PURCHASE_REGISTER_BANK_DETAIL (amount,accountNumber,voucherNumber,companyCode) values (?,?,?,?)";
            try {
                conn.setAutoCommit(false);
                ps = conn.prepareStatement(sql);
    ...
    public boolean saveItemOrderInfo(PurchaseRegister aThis, Connection conn, String companyCode, JTable itemTable) {
        boolean status = false;
        String sql2 = "insert into PURCHASE_REGISTER_ITEM_LIST (itemCode,unitName,quantity,rate,itemAmount,billSundry,taxClass,atTheRate,nature,miscAmount,totalRowAmount, expiryDate,voucherNumber,companyCode) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    public boolean saveBillSundryInfo(PurchaseRegister aThis, Connection conn, String companyCode, JTable billSundryTable) {
        boolean status = false;
        String sql = "insert into PURCHASE_REGISTER_BILL_SUNDRY_LIST (billSundry,atTheRate,taxClass,nature,amount,voucherNumber,companyCode) values (?,?,?,?,?,?,?)";
    ...
    public boolean deleteVoucherNumber(String companyCode, String voucherNumber, Connection conn) {
        boolean status = false;
        String sql = "delete from PURCHASE_REGISTER_DETAIL where voucherNumber = ? AND companyCode = ?";
        try {
            conn.setAutoCommit(false);
            ps = conn.prepareStatement(sql);
            ps.setString(1, voucherNumber);
    ...
    public boolean fetchDataOfPurchaseRegister(String companyCode, String voucherNumber, PurchaseRegister aThis, Connection conn) {
        boolean status = false;
        String sql = "select * from PURCHASE_REGISTER_DETAIL where voucherNumber = ? AND companyCode = ?";
        ...
        if (isCash) {
            status = false;
            String query = "select * from PURCHASE_REGISTER_CASH_DETAIL where voucherNumber = ? AND companyCode = ?";
            try {
                conn.setAutoCommit(false);
        ...
        if (isBank) {
            status = false;
            String query = "select * from PURCHASE_REGISTER_BANK_DETAIL where voucherNumber = ? AND companyCode = ?";
            try {
                conn.setAutoCommit(false);
                ps = conn.prepareStatement(query);
                ps.setString(1, voucherNumber);
    ...
    public boolean fetchItemDetail(String companyCode, String voucherNumber, JTable itemTable, PurchaseRegister aThis, Connection conn) {
        boolean status = false;
        String sql = "select * from PURCHASE_REGISTER_ITEM_LIST where voucherNumber = ? AND companyCode = ?";
        try {
            conn.setAutoCommit(false);
            ps = conn.prepareStatement(sql);
            ps.setString(1, voucherNumber);
    ...
    public boolean fetchBillSundryDetail(String companyCode, String voucherNumber, JTable billSundryTable, PurchaseRegister aThis, Connection conn) {
        boolean status = false;
        String sql = "select * from PURCHASE_REGISTER_BILL_SUNDRY_LIST where voucherNumber = ? AND companyCode = ?";
        try {
            conn.setAutoCommit(false);
            ps = conn.prepareStatement(sql);
            ps.setString(1, voucherNumber);
