 ...
 */
public class SaleRegisterHandler {

    private PreparedStatement ps;
    private ResultSet rs;
    private final MainFrame mainFrame;
    private final SaleRegister saleOrder;
     ...
     * @param saleOrder
     */
    public SaleRegisterHandler(MainFrame mainFrame, SaleRegister saleOrder) {
    ...
    public boolean fetchSaleRegisterInformation(String companyCode, JTable saleOrderListTable, Connection conn) {
        boolean status = false;
        removeAllRows(saleOrderListTable);
        String sql = "select * from SALE_REGISTER_DETAIL where companyCode = ?";
        ...
        try {
            ...
            Logger.getLogger(SaleRegisterHandler.class.getName()).log(Level.SEVERE, null, ex);
        }
        return status;
    }
    ...
    public boolean checkVoucherNumberIsValidOrNot(String companyCode, String voucherNumber, Connection conn) {
        boolean status = true;
        String sql = "select voucherNumber from SALE_REGISTER_DETAIL where voucherNumber = ? AND companyCode = ?";
        try {
            conn.setAutoCommit(false);
            ps = conn.prepareStatement(sql);
    ...
    public boolean saveSaleRegisterInfo(SaleRegister aThis, Connection conn, String companyCode) {
        boolean status = false;
        String sql1 = "insert into SALE_REGISTER_DETAIL values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        try {
            conn.setAutoCommit(false);
        ...
        if (aThis.getIsCash().equalsIgnoreCase("Y")) {
            status = false;
            String sql = "insert into SALE_REGISTER_CASH_DETAIL (amount,accountNumber,voucherNumber,companyCode) values (?,?,?,?)";
            try {
                conn.setAutoCommit(false);
                ps = conn.prepareStatement(sql);
        ...
        if (aThis.getIsBank().equalsIgnoreCase("Y")) {
            status = false;
            String sql = "insert into SALE_REGISTER_BANK_DETAIL (amount,accountNumber,voucherNumber,companyCode) values (?,?,?,?)";
            try {
                conn.setAutoCommit(false);
                ps = conn.prepareStatement(sql);
    ...
    public boolean saveItemOrderInfo(SaleRegister aThis, Connection conn, String companyCode, JTable itemTable) {
        boolean status = false;
        String sql2 = "insert into SALE_REGISTER_ITEM_LIST (itemCode,unitName,quantity,rate,itemAmount,billSundry,taxClass,atTheRate,nature,miscAmount,totalRowAmount, expiryDate,voucherNumber,companyCode) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    public boolean saveBillSundryInfo(SaleRegister aThis, Connection conn, String companyCode, JTable billSundryTable) {
        boolean status = false;
        String sql = "insert into SALE_REGISTER_BILL_SUNDRY_LIST (billSundry,atTheRate,taxClass,nature,amount,voucherNumber,companyCode) values (?,?,?,?,?,?,?)";
    ...
    public boolean deleteVoucherNumber(String companyCode, String voucherNumber, Connection conn) {
        boolean status = false;
        String sql = "delete from SALE_REGISTER_DETAIL where voucherNumber = ? AND companyCode = ?";
        try {
            conn.setAutoCommit(false);
            ps = conn.prepareStatement(sql);
            ps.setString(1, voucherNumber);
    ...
    public boolean fetchDataOfSaleRegister(String companyCode, String voucherNumber, SaleRegister aThis, Connection conn) {
        boolean status = false;
        String sql = "select * from SALE_REGISTER_DETAIL where voucherNumber = ? AND companyCode = ?";
        ...
        if (isCash) {
            status = false;
            String query = "select * from SALE_REGISTER_CASH_DETAIL where voucherNumber = ? AND companyCode = ?";
            try {
                conn.setAutoCommit(false);
        ...
        if (isBank) {
            status = false;
            String query = "select * from SALE_REGISTER_BANK_DETAIL where voucherNumber = ? AND companyCode = ?";
            try {
                conn.setAutoCommit(false);
                ps = conn.prepareStatement(query);
                ps.setString(1, voucherNumber);
    ...
    public boolean fetchItemDetail(String companyCode, String voucherNumber, JTable itemTable, SaleRegister aThis, Connection conn) {
        boolean status = false;
        String sql = "select * from SALE_REGISTER_ITEM_LIST where voucherNumber = ? AND companyCode = ?";
        try {
            conn.setAutoCommit(false);
            ps = conn.prepareStatement(sql);
            ps.setString(1, voucherNumber);
    ...
    public boolean fetchBillSundryDetail(String companyCode, String voucherNumber, JTable billSundryTable, SaleRegister aThis, Connection conn) {
        boolean status = false;
        String sql = "select * from SALE_REGISTER_BILL_SUNDRY_LIST where voucherNumber = ? AND companyCode = ?";
        try {
            conn.setAutoCommit(false);
            ps = conn.prepareStatement(sql);
            ps.setString(1, voucherNumber);
