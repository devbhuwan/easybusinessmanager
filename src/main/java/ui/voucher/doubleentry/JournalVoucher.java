...
import ui.reports.print.JournalVNotesTaker;
...
import ui.voucher.doubleentry.utils.JournalVoucherHandler;

/**
 *
 * @author Bhuwan Prasad Upadhyay
 */
public class JournalVoucher extends javax.swing.JPanel {
    ...
    private JournalVNotesTaker notesTaker;
    private VoucherPrint print;
    private String notes;

    /**
     * Creates new form JournalVoucher
     *
     * @param mainFrame
     * @param companyC
     * @param co
     */
    public JournalVoucher(MainFrame mainFrame, String companyC, Connection co) {
        initComponents();
    ...
    private void initComponents() {
        ...
        doubleEntryTable.setFont(new java.awt.Font("Trebuchet MS", 1, 12)); // NOI18N
        doubleEntryTable.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
                {null, null, null, null, null, null, null, null}
            },
            new String [] {
                "S. N.", "Acc. No.", "Acc. Name", "Amount", "Acc. No.", "Acc. Name", "Amount", "Narration"
            }
        ) {
            Class[] types = new Class [] {
    ...
    private void saveButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_saveButtonActionPerformed
        if (validateUserInput()) {
            if (new JournalVoucherHandler(mainFrame).checkVoucherNumberIsValidOrNot(companyCode, voucherNumberTF.getText(), conn)) {
                if (new JournalVoucherHandler(mainFrame).saveJournalVoucherInfo(this, conn, companyCode)) {
                    if (new JournalVoucherHandler(mainFrame).saveJournalVoucherAccountsInfo(this, conn, companyCode, doubleEntryTable)) {
    ...
    private void updateButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_updateButtonActionPerformed
        if (validateUserInput()) {
            if (voucherNumberTF.getText().equals(String.valueOf(voucherNumber))) {
                if (new JournalVoucherHandler(mainFrame).deleteVoucherNumber(companyCode, voucherNumber, conn)) {
                    if (new JournalVoucherHandler(mainFrame).saveJournalVoucherInfo(this, conn, companyCode)) {
                        if (new JournalVoucherHandler(mainFrame).saveJournalVoucherAccountsInfo(this, conn, companyCode, doubleEntryTable)) {
                            this.enableAllField(false);
                            this.saveOrUpdateControlShow();
    ...
    private void editButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_editButtonActionPerformed
        if (new JournalVoucherHandler(mainFrame).fetchJournalVoucherInformation(companyCode, doubleEntryListTable, conn)) {
            this.add(list);
    ...
    private void deleteButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_deleteButtonActionPerformed
        if (new JournalVoucherHandler(mainFrame).deleteVoucherNumber(companyCode, voucherNumberTF.getText(), conn)) {
            this.clearAllField();
            this.enableAllField(false);
...
notesTaker = new JournalVNotesTaker(mainFrame, this, true);
    ...
    private void doubleEntryListTableMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_doubleEntryListTableMouseClicked
        if (doubleEntryListTable.getRowCount() > 0) {
            if (evt.getClickCount() > 1 && doubleEntryListTable.getSelectedRow() > -1) {
                ...
                if (new JournalVoucherHandler(mainFrame).fetchDataOfJournalVoucher(companyCode, voucherNumber, this, conn)) {
                    if (new JournalVoucherHandler(mainFrame).fetchAccountDetail(companyCode, voucherNumber, doubleEntryTable, this, conn)) {
    ...
    public void setVoucherNumber(String companyCode, Connection conn) {
        voucherNumberTF.setText(new FetchVoucherData().getNextJournalVoucherNo(companyCode, conn));
    }
        ...
        if (selectedRow > -1) {
            if (doubleEntryTable.getSelectedRow() == doubleEntryTable.getRowCount() - 1) {
                ...
                switch (doubleEntryTable.getSelectedColumn()) {
                    case 1:
                        showListOfData("journaldebitaccno", doubleEntryTable, companyCode);
                        break;
                    case 2:
                        showListOfData("journaldebitaccname", doubleEntryTable, companyCode);
                        break;
                    case 3:
                        ...
                        break;
                    case 4:
                        showListOfData("journalcreditaccno", doubleEntryTable, companyCode);
                        break;
                    case 5:
                        showListOfData("journalcreditaccname", doubleEntryTable, companyCode);
                        break;
                    case 6:
