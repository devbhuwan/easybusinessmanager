...
import ui.voucher.doubleentry.JournalVoucher;

/**
 *
 * @author Bhuwan Upadhyay
 */
public class JournalVoucherHandler {
     ...
     * @param mainFrame
     */
    public JournalVoucherHandler(MainFrame mainFrame) {
    ...
    public boolean checkVoucherNumberIsValidOrNot(String companyCode, String voucherNumber, Connection conn) {
        boolean status = true;
        String sql = "select voucherNumber from JOURNAL_VOUCHER_DETAIL where voucherNumber = ? AND companyCode = ?";
        try {
    ...
    public boolean saveJournalVoucherInfo(JournalVoucher aThis, Connection conn, String companyCode) {
        boolean status = false;
        String sql1 = "insert into JOURNAL_VOUCHER_DETAIL values (?,?,?,?,?,?)";
        try {
            conn.setAutoCommit(false);
    ...
    public boolean saveJournalVoucherAccountsInfo(JournalVoucher aThis, Connection conn, String companyCode, JTable doubleEntryTable) {
        boolean status = false;
        String sql = "insert into JOURNAL_VOUCHER_ACCOUNTS_DETAIL_FOR_DEBIT (debitAccountNumber,debitAmount,narration,voucherNumber,companyCode) values (?,?,?,?,?)";
        String query = "insert into JOURNAL_VOUCHER_ACCOUNTS_DETAIL_FOR_CREDIT (creditAccountNumber,creditAmount,narration,voucherNumber,companyCode) values (?,?,?,?,?)";
    ...
    public boolean deleteVoucherNumber(String companyCode, String voucherNumber, Connection conn) {
        boolean status = false;
        String sql = "delete from JOURNAL_VOUCHER_DETAIL where voucherNumber = ? AND companyCode = ?";
    ...
    public boolean fetchJournalVoucherInformation(String companyCode, JTable saleOrderListTable, Connection conn) {
        boolean status = false;
        removeAllRows(saleOrderListTable);
        String sql = "select * from JOURNAL_VOUCHER_DETAIL where companyCode = ?";
    ...
    public boolean fetchDataOfJournalVoucher(String companyCode, String voucherNumber, JournalVoucher aThis, Connection conn) {
        boolean status = false;
        String sql = "select * from JOURNAL_VOUCHER_DETAIL where voucherNumber = ? AND companyCode = ?";
        try {
            conn.setAutoCommit(false);
            ps = conn.prepareStatement(sql);
            ps.setString(1, voucherNumber);
    ...
    public boolean fetchAccountDetail(String companyCode, String voucherNumber, JTable doubleEntryTable, JournalVoucher aThis, Connection conn) {
        boolean status = false;
        String sqlDebit = "select * from JOURNAL_VOUCHER_ACCOUNTS_DETAIL_FOR_DEBIT where voucherNumber = ? AND companyCode = ?";
        try {
            conn.setAutoCommit(false);
        ...
        String sqlCredit = "select * from JOURNAL_VOUCHER_ACCOUNTS_DETAIL_FOR_CREDIT where voucherNumber = ? AND companyCode = ?";
        try {
            conn.setAutoCommit(false);
            ...
            int rowCount = 0;
            while (rs.next()) {
                if (rowCount < doubleEntryTable.getRowCount() - 1) {
