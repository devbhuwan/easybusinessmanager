...
import ui.voucher.doubleentry.ReceiptVoucher;

/**
 *
 * @author Bhuwan Upadhyay
 */
public class ReceiptVoucherHandler {
     ...
     * @param mainFrame
     */
    public ReceiptVoucherHandler(MainFrame mainFrame) {
    ...
    public boolean checkVoucherNumberIsValidOrNot(String companyCode, String voucherNumber, Connection conn) {
        boolean status = true;
        String sql = "select voucherNumber from RECEIPT_VOUCHER_DETAIL where voucherNumber = ? AND companyCode = ?";
        try {
    ...
    public boolean saveReceiptVoucherInfo(ReceiptVoucher aThis, Connection conn, String companyCode) {
        boolean status = false;
        String sql1 = "insert into RECEIPT_VOUCHER_DETAIL values (?,?,?,?,?,?)";
        try {
            conn.setAutoCommit(false);
    ...
    public boolean saveReceiptVoucherAccountsInfo(ReceiptVoucher aThis, Connection conn, String companyCode, JTable doubleEntryTable) {
        boolean status = false;
        String sql = "insert into RECEIPT_VOUCHER_ACCOUNTS_DETAIL_FOR_DEBIT (debitAccountNumber,debitAmount,narration,voucherNumber,companyCode) values (?,?,?,?,?)";
        String query = "insert into RECEIPT_VOUCHER_ACCOUNTS_DETAIL_FOR_CREDIT (creditAccountNumber,creditAmount,narration,voucherNumber,companyCode) values (?,?,?,?,?)";
    ...
    public boolean deleteVoucherNumber(String companyCode, String voucherNumber, Connection conn) {
        boolean status = false;
        String sql = "delete from RECEIPT_VOUCHER_DETAIL where voucherNumber = ? AND companyCode = ?";
    ...
    public boolean fetchReceiptVoucherInformation(String companyCode, JTable saleOrderListTable, Connection conn) {
        boolean status = false;
        removeAllRows(saleOrderListTable);
        String sql = "select * from RECEIPT_VOUCHER_DETAIL where companyCode = ?";
    ...
    public boolean fetchDataOfReceiptVoucher(String companyCode, String voucherNumber, ReceiptVoucher aThis, Connection conn) {
        boolean status = false;
        String sql = "select * from RECEIPT_VOUCHER_DETAIL where voucherNumber = ? AND companyCode = ?";
        try {
            conn.setAutoCommit(false);
            ps = conn.prepareStatement(sql);
            ps.setString(1, voucherNumber);
    ...
    public boolean fetchAccountDetail(String companyCode, String voucherNumber, JTable doubleEntryTable, ReceiptVoucher aThis, Connection conn) {
        boolean status = false;
        String sqlDebit = "select * from RECEIPT_VOUCHER_ACCOUNTS_DETAIL_FOR_DEBIT where voucherNumber = ? AND companyCode = ?";
        try {
            conn.setAutoCommit(false);
        ...
        String sqlCredit = "select * from RECEIPT_VOUCHER_ACCOUNTS_DETAIL_FOR_CREDIT where voucherNumber = ? AND companyCode = ?";
        try {
            conn.setAutoCommit(false);
