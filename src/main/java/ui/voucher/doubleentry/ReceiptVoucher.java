...
import ui.reports.print.ReceiptNotesTaker;
...
import ui.voucher.doubleentry.utils.ReceiptVoucherHandler;

/**
 *
 * @author Bhuwan Prasad Upadhyay
 */
public class ReceiptVoucher extends javax.swing.JPanel {
    ...
    private ReceiptNotesTaker notesTaker;
     ...
     * @param co
     */
    public ReceiptVoucher(MainFrame mainFrame, String companyC, Connection co) {
        initComponents();
    ...
    private void saveButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_saveButtonActionPerformed
        if (validateUserInput()) {
            if (new ReceiptVoucherHandler(mainFrame).checkVoucherNumberIsValidOrNot(companyCode, voucherNumberTF.getText(), conn)) {
                if (new ReceiptVoucherHandler(mainFrame).saveReceiptVoucherInfo(this, conn, companyCode)) {
                    if (new ReceiptVoucherHandler(mainFrame).saveReceiptVoucherAccountsInfo(this, conn, companyCode, doubleEntryTable)) {
    ...
    private void updateButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_updateButtonActionPerformed
        if (validateUserInput()) {
            if (voucherNumberTF.getText().equals(String.valueOf(voucherNumber))) {
                if (new ReceiptVoucherHandler(mainFrame).deleteVoucherNumber(companyCode, voucherNumber, conn)) {
                    if (new ReceiptVoucherHandler(mainFrame).saveReceiptVoucherInfo(this, conn, companyCode)) {
                        if (new ReceiptVoucherHandler(mainFrame).saveReceiptVoucherAccountsInfo(this, conn, companyCode, doubleEntryTable)) {
                            this.enableAllField(false);
                            this.saveOrUpdateControlShow();
    ...
    private void editButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_editButtonActionPerformed
        if (new ReceiptVoucherHandler(mainFrame).fetchReceiptVoucherInformation(companyCode, doubleEntryListTable, conn)) {
            this.add(list);
    ...
    private void deleteButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_deleteButtonActionPerformed
        if (new ReceiptVoucherHandler(mainFrame).deleteVoucherNumber(companyCode, voucherNumberTF.getText(), conn)) {
            this.clearAllField();
            this.enableAllField(false);
    ...
    private void printButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_printButtonActionPerformed
        notesTaker = new ReceiptNotesTaker(mainFrame, this, true);
    ...
    private void doubleEntryListTableMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_doubleEntryListTableMouseClicked
        if (doubleEntryListTable.getRowCount() > 0) {
            if (evt.getClickCount() > 1 && doubleEntryListTable.getSelectedRow() > -1) {
                ...
                if (new ReceiptVoucherHandler(mainFrame).fetchDataOfReceiptVoucher(companyCode, voucherNumber, this, conn)) {
                    if (new ReceiptVoucherHandler(mainFrame).fetchAccountDetail(companyCode, voucherNumber, doubleEntryTable, this, conn)) {
    ...
    public void setVoucherNumber(String companyCode, Connection conn) {
        voucherNumberTF.setText(new FetchVoucherData().getNextReceiptVoucherNo(companyCode, conn));
    }
        ...
        if (selectedRow > -1) {
            if (doubleEntryTable.getSelectedRow() == doubleEntryTable.getRowCount() - 1) {
                ...
                switch (doubleEntryTable.getSelectedColumn()) {
                    case 1:
                        showListOfData("receiptdebitaccno", doubleEntryTable, companyCode);
                        break;
                    case 2:
                        showListOfData("receiptdebitaccname", doubleEntryTable, companyCode);
                        break;
                    case 3:
                        ...
                        break;
                    case 4:
                        showListOfData("receiptcreditaccno", doubleEntryTable, companyCode);
                        break;
                    case 5:
                        showListOfData("receiptcreditaccname", doubleEntryTable, companyCode);
                        break;
                    case 6:
